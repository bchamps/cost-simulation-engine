...
UF_MAPPING = [
    ...
    ('PB', 'Northeast', 0.01), ('RN', 'Northeast', 0.01), ('AL', 'Northeast',
                                                           0.01), ('PI', 'Northeast', 0.01), ('SE', 'Northeast', 0.01),
...
HIST_DATES = pd.date_range(
    start=f'{START_HIST_YEAR}-01-01',
    end=f'{END_HIST_YEAR}-12-01',
...
def generate_dim_scenario():
    scenarios = [
        ...
        ('rolling_forecast_q3', 'Rolling Forecast Q3', 'admin',
         '2025-09-01 00:00:00', 'Updated rolling projection', False)
    ]
    df = pd.DataFrame(
        scenarios,
        columns=[
...
def generate_fact_pl_monthly(df_pos):
    ...
    idx = pd.MultiIndex.from_product([df_pos['pos_id'], ALL_DATES, SCENARIOS],
                                     names=['pos_id', 'year_month', 'scenario_id'])
    ...
    base_rev = np.random.lognormal(mean=10.2, sigma=0.4, size=len(df_fact)) * \
        size_mult * sp_outlier * seasonality * lrp_compounding
